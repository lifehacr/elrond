...
import { Blog, LatestPostsType, Media, Tag, User } from '@payload-types'
import Image from 'next/image'
import Link from 'next/link'
import { useRouter } from 'next/navigation'
import React, { useState } from 'react'

const LatestPostsClient: React.FC<{
  blogs: Blog[]
  tags: Tag[]
  block: LatestPostsType
}> = ({ blogs, tags, block }) => {
  const [noOfPosts, setNoOfPosts] = useState<number>(6)
  const router = useRouter()

  const loadPosts = () => {
    setNoOfPosts(prev => Math.min(prev + 6, blogs.length))
  }
      ...
      <div className='relative mx-auto my-4 flex w-full flex-col items-start justify-between gap-16 sm:my-12 md:max-w-[940px] lg:my-16 lg:flex-row lg:gap-16'>
        <div className='flex flex-col gap-4 md:mx-0'>
          ...
          <div className='flex w-full flex-col gap-10'>
            {blogs?.slice(0, noOfPosts).map((blog, index) => (
              <div
                key={index}
          ...
          {blogs?.length! > noOfPosts && (
            <div className='mt-8 flex items-center justify-center'>
              <Button
                onClick={loadPosts}
                className='rounded-lg border border-zinc-200 bg-secondary px-3 py-[2px] text-xs font-semibold text-secondary-content hover:bg-secondary hover:opacity-80'
                disabled={noOfPosts >= blogs.length}>
                Load More
              </Button>
            </div>
          )}
        ...
        <div className='relative my-8 flex h-full w-full flex-shrink-0 basis-full flex-col items-center gap-10 sm:my-10 md:sticky md:top-6 md:my-0 md:basis-[30%] md:items-start'>
          ...
          <div className='flex h-fit w-full max-w-md flex-col gap-4'>
            ...
            <div className='flex flex-col gap-3'>
              ...
              <div>
                <button
                  onClick={() => router.push(block?.buttonPath)}
...
export default LatestPostsClient
